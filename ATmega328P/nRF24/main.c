                    ...
                    uint8_t *val, uint8_t antVal)
{
    ...
    //CSN goes High, and doing nothing
    PORTB|=_BV(csnPin);
...
void nRF24L01_init(void)
{
    ...
    //USART_Tx_asciiCode(GetReg(TX_ADDR));
    //payload width setup -
    //How many bytes to send per transmission? 1-32bytes
    val[0]=dataLen;
    WriteToNrf(W, RX_PW_P0, val, 1);
    //USART_Tx_asciiCode(GetReg(RX_PW_P0));
    ...
    USART_Transmit('S');
    USART_Tx_asciiCode(GetReg(STATUS));
    //sei();
}//nRF24L01_init
...
int main(void)
{
    ...
    while(1)
    {
        ...
        val = PIND & _BV(swPin);
        //USART_Tx_asciiCode('v');
        if (val == 0)
        {
            //USART_Tx_asciiCode(val);
            sendSig();
