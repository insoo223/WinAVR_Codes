...
void dispTest()
{
    ...
    {
        //PORTB ^= _BV(PB0);
        show7seg(OnesDigit[i % 10]); //Ones
        show7seg(TensDigit[i / 10]); //Tens
        //_delay_ms(170);
        _delay_ms(75);
    }
...
int8_t dht_getdata(int8_t *temperature, int8_t *humidity)
{
    ...
    //DHT_PORT |= _BV(switch_PIN); //high debug pin
    //_delay_us(20);
    //DHT_PORT &= ~_BV(switch_PIN); //low debug pin
